 ...
 * </p>
 */
public class DoubleDateTimeDialog extends AlertDialog implements OnClickListener, OnDateChangedListener,OnTimeChangedListener {
	...
	public interface OnDateSetListener {
		void onDateSet(DatePicker startDatePicker, int startYear, int startMonthOfYear, int startDayOfMonth, TimePicker timepicker, int hour, int min);
	}
	 ...
	 */
	public DoubleDateTimeDialog(Context context, OnDateSetListener callBack, int year, int monthOfYear, int dayOfMonth,int hour,int min) {
		this(context, 0, callBack, year, monthOfYear, dayOfMonth,hour,min);
	}

	public DoubleDateTimeDialog(Context context, int theme, OnDateSetListener callBack, int year, int monthOfYear,
			int dayOfMonth,int hour,int min) {
		this(context, 0, callBack, year, monthOfYear, dayOfMonth, true,hour,min);
	}
	 ...
	 */
	public DoubleDateTimeDialog(Context context, int theme, OnDateSetListener callBack, int year, int monthOfYear,
			int dayOfMonth, boolean isDayVisible,int hour,int min) {
		super(context, theme);
	...
	private void tryNotifyDateSet() {
		if (mCallBack != null) {
			...
			mCallBack.onDateSet(mDatePicker_start, mDatePicker_start.getYear(), mDatePicker_start.getMonth(),
					mDatePicker_start.getDayOfMonth(),timepicker,timepicker.getCurrentHour(),timepicker.getCurrentMinute());
		}
	}
...
//		int end_day = savedInstanceState.getInt(END_DAY);
	...
	private void resizeNumberPicker(NumberPicker np){
		LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(55, LinearLayout.LayoutParams.WRAP_CONTENT);
	...
	public void onTimeChanged(TimePicker arg0, int arg1, int arg2) {
		...
		if (arg0.getId() == R.id.datePickerStart)
		{	timepicker.setCurrentHour(arg1);
		timepicker.setCurrentMinute(arg2);
